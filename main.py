                ...
                if os.path.exists(main_file):
                    ...
                    for name, obj in module.__dict__.items():
                        if isinstance(obj, type) and issubclass(obj, BaseAction) and obj is not BaseAction:
                            print(f"\nInitializing action: Voyxa AI Voice Assistent")
                            action_instance = obj(self)

    async def run_transcription_and_completion(self, filename):
        """Run transcription and completion in parallel."""
        loop = asyncio.get_event_loop()
        transcription_task = loop.run_in_executor(self.executor, self.transcription_manager.transcribe, filename)
        completion_task = loop.run_in_executor(self.executor, self.completion_client.complete, transcription_task)

        transcription = await transcription_task
        completion = await completion_task

        return transcription, completion

    def run(self):
        """Run the Voyxa instance, setting up hotkeys and entering the main loop."""
        print("\n\nSetting up Voyxa...\n")
        ...
        print("\nVoyxa is reddy. Use any of the hotkeys above to get started.")
        try:
...
if __name__ == "__main__":
    try:
        ...
        if config.VERBOSE:
            ...
            print(f"Failed to start Voyxa: {e}")
